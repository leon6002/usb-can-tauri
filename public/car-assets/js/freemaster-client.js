...
(function(root) {
  ...
  var PCM = function(url, onSocketOpen, onSocketClose, onSocketError) {
    ...
    this._jrpc.dispatch('OnCustomEvent', 'pass', (params_array) => this.OnCustomEvent.apply(null, params_array));
  };
   ...
   *
   * @example
   * pcm.WriteMultipleVariables([{ name: "var8", value: 255 }, { name: "varFLT", value: 3.14 }]).then(() => console.log(data));
   *
   ...
   *
   * @param   {boolean} saveAs  When false, project is silently saved to last known path.
   ...
   *
   * @example
   * pcm.DefineArrayViewer(name, defStr).then(() => console.log("Info is " + result.data));
   *
   * @param   {string} name Name of item.
   * @param   {string} defStr Stringified JSON definition record. Refer to FreeMASTER Desktop app. documentation for more details.
   * @returns {Promise} The result does not carry any relevant data.
   ...
   *
   * @example
   * pcm.DefinePipe(name, defStr).then(() => console.log("Info is " + result.data));
   *
   * @param   {string} name Name of item.
   * @param   {string} defStr Stringified JSON definition record. Refer to FreeMASTER Desktop app. documentation for more details.
   * @returns {Promise} The result does not carry any relevant data.
   ...
   *
   * @example
   * pcm.DefineWatchBlock(name, defStr).then(() => console.log("Info is " + result.data));
   *
   * @param   {string} name Name of item.
